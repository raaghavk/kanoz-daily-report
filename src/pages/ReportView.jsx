              ...
              {equipmentDiesel.length > 0 ? (
                equipmentDiesel.map(e => (
                  <tr key={e.id} style={{ borderTop: '1px solid #e5ddd0' }}>
                    ...
                    <td style={{ padding: '10px 12px', textAlign: 'right', color: '#595c4a', fontSize: 11 }}>{Math.round(e.opening_litres || 0)}</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', color: '#595c4a', fontSize: 11 }}>{Math.round(e.added_litres || 0)}</td>
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontWeight: 700, color: '#2c2c2c', fontSize: 11 }}>{Math.round(e.closing_litres || 0)}</td>
        ...
        <div style={{ padding: '0 20px', marginTop: 24 }}>
          <div style={{ background: '#f5f0e1', borderRadius: 14, padding: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {can(employee?.role, 'export') && (
              <button
                onClick={() => exportShiftReportPDF({ report, machineProduction, rawMaterials, equipmentDiesel, pelletStock, dispatches, issues, dieselStock, createdByName })}
                style={{
