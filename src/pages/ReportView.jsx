  ...
  async function syncReport() {
    ...
    try {
      ...
      if (rmUsageRows && rmUsageRows.length > 0) {
        ...
        if (purchases) { for (const p of purchases) { purchasedByType[p.raw_material_type_id] = (purchasedByType[p.raw_material_type_id] || 0) + (parseFloat(p.net_weight) || 0) } }
        for (const row of rmUsageRows) {
          const newPurchased = purchasedByType[row.raw_material_type_id] || 0
          const { data: cur } = await supabase.from('raw_material_usage').select('opening_kg, quantity_kg').eq('id', row.id).single()
          if (cur) {
            const newClosing = (parseFloat(cur.opening_kg) || 0) + newPurchased - (parseFloat(cur.quantity_kg) || 0)
            await supabase.from('raw_material_usage').update({ purchased_kg: newPurchased, closing_kg: newClosing }).eq('id', row.id)
          }
        }
      }
...
${report.handover_notes ? `<div class="section"><h2>Handover Notes</h2><p>${report.handover_notes}</p></div>` : ''}
  ...
  return (
    <div style={{ minHeight: '100%', background: '#fefae0', paddingBottom: 80 }}>
      ...
      <div style={{ padding: '0 20px', marginTop: 24 }}>
        ...
        <div style={{ marginTop: 12, background: '#fefae0', borderRadius: 12, border: '1.5px solid #e9c46a', padding: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, textAlign: 'center' }}>
            {[
              { label: 'Opening (L)', value: Math.round((dieselStock?.opening_litres || 0) * 100) / 100 || undefined ?? '-' },
      ...
      {/* Created By Info */}
      <div style={{ padding: '0 20px', marginTop: 24 }}>
        <div style={{ background: '#f5f3eb', borderRadius: 14, padding: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 10, color: '#8a8d7a', fontWeight: 600 }}>Created by</div>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#2c2c2c', marginTop: 2 }}>{createdByName || report.employees?.name || 'Unknown'}</div>
            <div style={{ fontSize: 10, color: '#8a8d7a', marginTop: 1 }}>
              {report.created_at ? new Date(report.created_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true }) : ''}
            </div>
          </div>
        </div>
      </div>
