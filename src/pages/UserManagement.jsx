...
import { UserPlus, Edit2, Shield, ShieldOff, ChevronLeft, Phone, Mail, MapPin, Check, X, Loader2, Users, KeyRound, Eye, EyeOff } from 'lucide-react'
...
export default function UserManagement() {
  ...
  const [showSetPwd, setShowSetPwd] = useState(null) // employee id
  ...
  async function setUserPassword() {
    const emp = employees.find(e => e.id === showSetPwd)
    ...
    if (!emp.auth_user_id && !pwdForm.email.trim()) { showToast('Email is required', 'error'); return }

    setSettingPwd(true)
    try {
      const { data, error } = await supabase.functions.invoke('set-user-password', {
        body: {
          employee_id: showSetPwd,
      ...
      showToast(emp.auth_user_id ? 'Password updated!' : 'Account created with password!')
      setShowSetPwd(null)
  ...
  const inputStyle = {
    ...
    border: '1.5px solid #e5ddd0', background: 'white', color: '#2c2c2c',
    fontSize: 14, outline: 'none'
  }

  const labelStyle = { display: 'block', fontSize: 12, fontWeight: 600, color: '#595c4a', marginBottom: 6 }

  if (loading) {
    return (
      <div style={{ padding: 20, textAlign: 'center', paddingTop: 80 }}>
        <Loader2 size={32} color="#2d6a4f" style={{ animation: 'spin 1s linear infinite', margin: '0 auto' }} />
        <p style={{ fontSize: 14, color: '#595c4a', marginTop: 12 }}>Loading...</p>
      </div>
    )
  }

  return (
    <div style={{ minHeight: '100vh', background: '#f5edd6', display: 'flex', justifyContent: 'center' }}>
    <div style={{ minHeight: '100vh', background: '#fefae0', width: '100%', maxWidth: 480, boxShadow: '0 0 40px rgba(0,0,0,0.08)' }}>
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg, #2d6a4f 0%, #15603A 100%)', color: 'white', padding: '16px 20px', paddingTop: 'max(16px, env(safe-area-inset-top))' }}>
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            width: '100%', padding: '12px 16px', background: '#2d6a4f', color: 'white',
        ...
        {employees.map(emp => (
          <div key={emp.id} style={{
            background: 'white', borderRadius: 14, border: '1.5px solid #e5ddd0',
            padding: 16, opacity: emp.is_active ? 1 : 0.6
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span style={{ fontSize: 16, fontWeight: 700, color: '#2c2c2c' }}>{emp.name}</span>
                  {!emp.is_active && (
                    <span style={{ fontSize: 10, background: '#FED7D7', color: '#d32f2f', padding: '2px 8px', borderRadius: 20, fontWeight: 600 }}>Inactive</span>
                  )}
                </div>
                ...
                {emp.mobile && (
                  <div style={{ fontSize: 12, color: '#595c4a', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button
                  ...
                  style={{
                    width: 36, height: 36, borderRadius: 10, border: '1.5px solid #e5ddd0',
                    background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer'
                  }}
                >
                  <Edit2 size={16} color="#595c4a" />
                </button>
                {/* Key icon — set/change password */}
                <button
                  onClick={() => { setShowSetPwd(emp.id); setPwdForm({ email: '', password: '' }); setShowPwd(false) }}
                  title={emp.auth_user_id ? 'Change password' : 'Set password'}
                  style={{
                    width: 36, height: 36, borderRadius: 10, border: '1.5px solid #e5ddd0',
                    background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer'
                  }}
                >
                  <KeyRound size={16} color="#595c4a" />
                </button>
                {!emp.auth_user_id && (
                  <button
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, padding: 24, maxHeight: '85vh', overflow: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={labelStyle}>Name <span style={{ color: '#d32f2f' }}>*</span></label>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: form.is_active ? '#2d6a4f' : '#D1D5DB',
              ...
              <button
                ...
                disabled={saving}
                style={{
                  width: '100%', padding: '14px 0', background: '#2d6a4f', color: 'white',
      ...
      {showInvite && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, padding: 24 }}>
            ...
            <p style={{ fontSize: 13, color: '#595c4a', marginBottom: 16 }}>
            ...
            <div style={{ background: 'white', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 480, padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>
                  {emp?.auth_user_id ? 'Change Password' : 'Set Password'}
                </h2>
                <button onClick={() => { setShowSetPwd(null); setShowPwd(false) }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}>
                  <X size={24} color="#595c4a" />
                </button>
              </div>
              <p style={{ fontSize: 13, color: '#595c4a', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    >
                      {showPwd ? <EyeOff size={18} color="#8a8d7a" /> : <Eye size={18} color="#8a8d7a" />}
                    </button>
                  </div>
                </div>
                ...
                <button
                  ...
                  style={{
                    width: '100%', padding: '14px 0', background: '#2d6a4f', color: 'white',
                    borderRadius: 12, fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer',
                    opacity: settingPwd ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8
                  }}
                >
                  <KeyRound size={16} />
                  {settingPwd ? 'Saving...' : emp?.auth_user_id ? 'Update Password' : 'Create Account & Set Password'}
                </button>
              </div>
            </div>
          </div>
      ...
      {toast && (
        <div style={{
          ...
          background: toast.type === 'error' ? '#d32f2f' : '#2d6a4f', color: 'white',
